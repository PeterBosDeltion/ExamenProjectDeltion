...
namespace Pinwheel.Griffin.DataTool
{
    public class GTextureExporter
    {
        ...
        private void DoExportHeightMap()
        {
            ...
            Color[] colors = SrcData.Geometry.Internal_HeightMap.GetPixels();
            for (int i = 0; i < colors.Length; ++i)
            {
                colors[i] = new Color(colors[i].r, 0, 0, 0);
            }
        ...
        {
            GUtilities.EnsureDirectoryExists(DataDirectory);
            byte[] data = SrcData.Shading.Internal_AlbedoMap.EncodeToPNG();
        ...
        {
            GUtilities.EnsureDirectoryExists(DataDirectory);
            byte[] data = SrcData.Shading.Internal_MetallicMap.EncodeToPNG();
        ...
        private void DoExportGradientLookupMaps()
        {
            ...
            {
                SrcData.Shading.Internal_ColorByHeightMap,
                SrcData.Shading.Internal_ColorByNormalMap,
                SrcData.Shading.Internal_ColorBlendMap
            };
        ...
        private void DoExportSplatControlMaps()
        {
            ...
            {
                Texture2D tex = SrcData.Shading.Internal_GetSplatControl(i);
                byte[] data = tex.EncodeToPNG();
                string fileName = string.Format("{0}_{1}x{2}_{3}.png", tex.name.Replace(" ", ""), tex.width, tex.height, SrcData.Id);
                string filePath = Path.Combine(DataDirectory, fileName);
                File.WriteAllBytes(filePath, data);
