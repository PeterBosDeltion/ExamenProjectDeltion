    ...
    public class GTextureImporter
    {
        ...
        public void Import()
        {
            try
            {
                ...
                {
                    Color[] oldHeightMapColors = DesData.Geometry.Internal_HeightMap.GetPixels();
                    ...
                    for (int y = 0; y < desResolution; ++y)
                    {
                        for (int x = 0; x < desResolution; ++x)
                        {
                            ...
                            if (hm != null)
                            {
                                c.r = hm.GetPixelBilinear(uv.x, uv.y).r;
                            }
                            else
                            {
                                c.r = DesData.Geometry.Internal_HeightMap.GetPixelBilinear(uv.x, uv.y).r;
                            }
                            if (vm != null)
                            {
                            ...
                            {
                                c.a = DesData.Geometry.Internal_HeightMap.GetPixelBilinear(uv.x, uv.y).a;
                            }
                    ...
                    DesData.Geometry.Internal_HeightMap.SetPixels(newColor);
                    ...
                    Color[] newHeightMapColors = DesData.Geometry.Internal_HeightMap.GetPixels();
                ...
                if (AlbedoMap != null)
                {
                    GCommon.CopyTexture(AlbedoMap, DesData.Shading.Internal_AlbedoMap);
                    DesData.SetDirty(GTerrainData.DirtyFlags.Shading);
                }
                if (MetallicMap != null)
                {
                    GCommon.CopyTexture(MetallicMap, DesData.Shading.Internal_MetallicMap);
                    DesData.SetDirty(GTerrainData.DirtyFlags.Shading);
                }
                if (SplatControlMaps != null)
                {
                    ...
                    for (int i = 0; i < count; ++i)
                    {
                        ...
                        {
                            GCommon.CopyTexture(SplatControlMaps[i], DesData.Shading.Internal_GetSplatControl(i));
                        }
                    }
                    DesData.SetDirty(GTerrainData.DirtyFlags.Shading);
                }
