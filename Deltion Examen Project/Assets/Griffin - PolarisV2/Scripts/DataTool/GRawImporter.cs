...
namespace Pinwheel.Griffin.DataTool
{
    public class GRawImporter
    {
        ...
        private void ImportRaw8()
        {
            ...
            Color[] heightMapColors = DesData.Geometry.Internal_HeightMap.GetPixels();
            ...
            for (int z = 0; z < heightMapResolution; ++z)
            {
                for (int x = 0; x < heightMapResolution; ++x)
                {
                    ...
                    c.r = GUtilities.GetValueBilinear(heightData, rawResolution, rawResolution, uv);
                    heightMapColors[GUtilities.To1DIndex(x, z, heightMapResolution)] = c;
                }
            }

            DesData.Geometry.Internal_HeightMap.SetPixels(heightMapColors);
