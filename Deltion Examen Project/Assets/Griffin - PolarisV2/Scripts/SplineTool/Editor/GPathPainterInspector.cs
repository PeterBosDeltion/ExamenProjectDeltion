...
namespace Pinwheel.Griffin.SplineTool
{
    ...
    public class GPathPainterInspector : Editor
    {
        ...
        private void DrawLivePreview(GStylizedTerrain t, Camera cam)
        {
            if (instance.Channel == GPathPainter.PaintChannel.AlbedoAndMetallic)
            {
                SetupAlbedoMetallicPreview(t);
                DrawTerrainMeshPreview(t, cam);
            }
            else if (instance.Channel == GPathPainter.PaintChannel.Splat)
            {
                SetupSplatPreview(t);
        ...
        private void SetupAlbedoMetallicPreview(GStylizedTerrain t)
        {
            ...
            {
                previewPropertyBlock.SetTexture(t.TerrainData.Shading.AlbedoMapPropertyName, rtAlbedo);
            }
            if (!string.IsNullOrEmpty(t.TerrainData.Shading.MetallicMapPropertyName))
            {
                previewPropertyBlock.SetTexture(t.TerrainData.Shading.MetallicMapPropertyName, rtMetallic);
            }
        }
        ...
        private void SetupSplatPreview(GStylizedTerrain t)
        {
            ...
            {
                rtControls[i] = GetPreviewTexture(t, "splatControl" + i, controlMapResolution, t.TerrainData.Shading.Internal_SplatControls[i].filterMode);
            }
