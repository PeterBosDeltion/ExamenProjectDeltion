...
namespace Pinwheel.Griffin
{
    public class GFoliage : ScriptableObject
    {
        ...
        {
            int count = 0;
            List<string> keys = TerrainData.FoliageData.GetKeys();
            for (int i = 0; i < keys.Count; ++i)
            {
                bool delete = false;
                if (!keys[i].StartsWith("~GrassPatch"))
                {
                    delete = false;
                }
                else
                {
                    try
                    {
                        ...
                        int prototype = int.Parse(splits[3]);
        ...
        private void CalculateEstimatedGrassStorage()
        {
            ...
            for (int i = 0; i < instances.Count; ++i)
            {
                ...
                if (m == null)
                    continue;
                byteCount += 2 * (vertexCountPerPrototypes[protoIndex] * 60 + indexCountPerPrototypes[protoIndex] * 4);
            }
