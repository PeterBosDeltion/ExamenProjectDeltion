...
namespace Pinwheel.Griffin
{
    ...
    public class GTerrainGeneratedDataInspector : Editor
    {
        ...
        [MenuItem("CONTEXT/GTerrainGeneratedData/Clear Geometry Meshes (Internal)")]
        private static void ClearGeometryMeshes()
        {
            Object o = Selection.activeObject;
            if (o is GTerrainGeneratedData)
            {
                GTerrainGeneratedData data = o as GTerrainGeneratedData;
                data.Internal_DeleteMeshIf((k) => { return k.StartsWith(GCommon.CHUNK_MESH_NAME_PREFIX); });
                AssetDatabase.Refresh();
            }
        }

        [MenuItem("CONTEXT/GTerrainGeneratedData/Clear Grass Meshes (Internal)")]
