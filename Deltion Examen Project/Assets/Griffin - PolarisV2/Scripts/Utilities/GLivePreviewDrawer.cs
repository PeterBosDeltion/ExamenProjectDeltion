    ...
    public static class GLivePreviewDrawer
    {
        ...
        public static void DrawSubdivLivePreview(GStylizedTerrain t, Camera cam, Texture newHeightMap, Rect dirtyRect, Texture mask, Matrix4x4 worldPointToMaskMatrix)
        {
            ...
            Vector3 terrainSize = new Vector3(
                ...
                t.TerrainData.Geometry.Length);
            previewPropertyBlock.Clear();
            previewPropertyBlock.SetColor("_Color", new Color(0, 0, 0, 0.85f));
            previewPropertyBlock.SetTexture("_MainTex", Texture2D.whiteTexture);
            previewPropertyBlock.SetFloat("_Thickness", 0.75f);
            ...
            for (int x = 0; x < gridSize; ++x)
            {
                for (int z = 0; z < gridSize; ++z)
                {
                    ...
                    {
                        previewMesh = GGriffinSettings.Instance.GetLivePreviewMesh(i);
