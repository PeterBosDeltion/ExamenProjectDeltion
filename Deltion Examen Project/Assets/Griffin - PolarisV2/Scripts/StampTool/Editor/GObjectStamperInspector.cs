        ...
        private void DrawLivePreview(GStylizedTerrain t, Camera cam)
        {
            ...
            for (int i = 0; i < chunks.Length; ++i)
            {
                ...
                if (uvRange.Overlaps(dirtyRect))
                {
                    ...
                    if (chunkMesh != null)
                    {
                        for (int layer = 0; layer < instance.Layers.Count; ++layer)
                        {
                            previewPropertyBlock.Clear();
                            previewPropertyBlock.SetColor("_Color", instance.Layers[layer].VisualizeColor);
                            previewPropertyBlock.SetTexture("_MainTex", brushes[layer]);
