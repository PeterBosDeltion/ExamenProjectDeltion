...
namespace Pinwheel.Griffin
{
    public static partial class GCommonGUI
    {
        ...
        public static int _SelectionGrid(_GSelectionGridArgs args)
        {
            ...
            EditorGUILayout.BeginHorizontal();
            EditorGUILayout.GetControlRect(GUILayout.Width(EditorGUI.indentLevel * GCommonGUI.indentSpace));
            ...
            for (int i = 0; i < itemPerRow * numberOfRow; ++i)
            {
                ...
                if (i < args.collection.Count)
                {
                    ...
                    if (args.selectedIndex == currentItemIndex)
                    {
                        ...
                        EditorGUI.DrawRect(highlightRect, GCommonGUI.selectedItemColor);
                    }
        ...
        public static T ObjectSelectorDragDrop<T>(Rect r, string message, string filter = "") where T : class
        {
            ...
            Color bgColor = EditorGUIUtility.isProSkin ? GCommonGUI.darkGrey : GCommonGUI.lightGrey;


            EditorGUI.DrawRect(r, bgColor);
            GCommonGUI.DrawOutlineBox(r, GCommonGUI.midGrey);

            Rect messageRect = new Rect();
            messageRect.size = new Vector2(r.width, 12);
            messageRect.center = r.center - Vector2.up * (messageRect.size.y * 0.5f + 2);
            EditorGUI.LabelField(messageRect, message, GCommonGUI.CenteredLabel);

            Rect buttonRect = new Rect();
            buttonRect.size = new Vector2(47, 15);
            buttonRect.center = r.center + Vector2.up * (buttonRect.size.y * 0.5f + 2);
            if (GUI.Button(buttonRect, "Browse", GCommonGUI.CenteredLabel))
            {
                ...
                e.type == EventType.Repaint))
            {
                    ...
                    draggedObject[0] is T)
                {
                    ...
                    EditorGUI.DrawRect(r, GUtilities.GetColor(GCommonGUI.selectedItemColor, 0.5f));
                    GCommonGUI.DrawOutlineBox(r, GCommonGUI.selectedItemColor);
                }
            }
        ...
        public static int SplatSetSelectionGrid(int groupId, int selectedIndex)
        {
            ...
            if (set != null && set.Prototypes.Count > 0)
            {
                ...
                args.itemSize = GCommonGUI.selectionGridTileSizeSmall;
                args.itemPerRow = 4;
                args.drawPreviewFunction = DrawSplatPreview;
                selectedIndex = GCommonGUI.SelectionGrid(args);
            }
            else
            {
        ...
        public static List<int> TreeSetMultiSelectionGrid(int groupId, List<int> selectedIndices)
        {
            ...
            if (set != null && set.Prototypes.Count > 0)
            {
                ...
                args.itemPerRow = 4;
                args.drawPreviewFunction = DrawTreePreview;
                selectedIndices = GCommonGUI.MultiSelectionGrid(args);
            }
            else
            {
                EditorGUILayout.LabelField("No Tree found in this set!", ItalicLabel);
        ...
        public static void DrawTreePreview(Rect r, object o)
        {
            ...
            if (t != null)
            {
                ...
                else
                {
                    ...
                    EditorGUI.LabelField(r, "...", GCommonGUI.CenteredWhiteLabel);
                }
            }
        }
        ...
        {
            string prefKey = GCommonGUI.GetProjectRelatedEditorPrefsKey("foldout", prefkeys);
        ...
        public static Rect BeginGroup()
        {
            ...
            GUI.Box(GCommonGUI.boxOffset.Add(r), "");
            return r;
        }
        ...
        {
            string prefKey = GCommonGUI.GetProjectRelatedEditorPrefsKey("foldout", id);
        ...
        public static bool Foldout(string label, bool defaultExpanded, string id, Action innerGUI, GenericMenu context = null)
        {
            ...
            GUI.Box(headerBoxOffset.Add(headerRect), "");
            ...
            //{
            //    Color overlayColor = EditorGUIUtility.isProSkin ? new Color(1, 1, 1, 0.1f) : new Color(0, 0, 0, 0.1f);
            //    EditorGUI.DrawRect(headerBoxOffset.Add(headerRect), overlayColor);
            //}
            ...
            //AnimBool a = GetFoldoutState(id, expanded);
            ...
            if (expanded)
            {
                ...
                GUI.Box(bodyBoxOffset.Add(bodyRect), "");
            }

            //EditorGUILayout.BeginFadeGroup(a.faded);
            //if (a.faded > 0)
            if (expanded)
            {
        ...
        {
            EditorGUI.DrawRect(r, GCommonGUI.assetPreviewGrey);
            ...
            else
            {
                ...
                {
                    EditorGUI.LabelField(r, "Loading preview...", GCommonGUI.CenteredWhiteLabel);
                }
                else
                {
                    EditorGUI.LabelField(r, "Preview not available!", GCommonGUI.CenteredWhiteLabel);
                }
            }
        }
        ...
        private static List<int> MultiSelectionGridInternal(GSelectionGridArgs args)
        {
            ...
            while (currentIndex < args.collection.Count)
            {
                ...
                for (int i = 0; i < itemPerRow; ++i)
                {
                    if (iter.MoveNext())
                    {
                        ...
                        {
                            Color c = GCommonGUI.selectedItemColor;
            ...
            if (!args.simpleMode)
            {
                EditorGUILayout.LabelField("Use Shift/Ctrl & Left Click to set probabilities.", GCommonGUI.WordWrapItalicLabel);
            }
            ...
            while (currentIndex < args.collection.Count)
            {
                ...
                for (int i = 0; i < itemPerRow; ++i)
                {
                    if (iter.MoveNext())
                    {
                        ...
                        {
                            EditorGUI.DrawRect(itemRect, GCommonGUI.selectedItemColor);
                            previewRect = new RectOffset(2, 2, 2, 2).Remove(previewRect);
                        }
        ...
        public static void ProgressBar(string title, string detail, float percent)
        {
            EditorUtility.DisplayProgressBar(title, detail, percent);
        }

        public static void CancelableProgressBar(string title, string detail, float percent)
        {
