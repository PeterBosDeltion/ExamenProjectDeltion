...
namespace Pinwheel.Griffin
{
    ...
    public class GStylizedTerrain : MonoBehaviour
    {
        ...
        internal Transform Internal_ChunkRoot
        {
            get
            {
        ...
        private void OnGeometryDirty()
        {
            if (PreProcessHeightMap != null)
                PreProcessHeightMap.Invoke(TerrainData.Geometry.Internal_HeightMap);
            try
            {
                InitChunks();
                TerrainData.Geometry.Internal_CreateNewSubDivisionMap();

                GTerrainChunk[] chunks = Internal_GetChunks();
            ...
            {
                PostProcessHeightMap.Invoke(TerrainData.Geometry.Internal_HeightMap);
            }
        }
        ...
        public GTerrainChunk[] Internal_GetChunks()
        {
        ...
        {
            List<GTerrainChunk> chunks = new List<GTerrainChunk>(Internal_GetChunks());
            chunks.Sort((c1, c2) =>
            {
        ...
        public Vector4 GetHeightMapSample(Vector2 uv)
        {
            if (TerrainData == null)
                return Vector4.zero;
            Vector4 sample = TerrainData.Geometry.Internal_HeightMap.GetPixelBilinear(uv.x, uv.y);
            return sample;
        }
        ...
        public float GetInterpolatedGeometryHeightSample(Vector2 pos)
        {
            if (TerrainData == null)
                return 0;
            if (heightMap2D == null)
            ...
            if (heightMap2D.width != TerrainData.Geometry.HeightMapResolution ||
                heightMap2D.height != TerrainData.Geometry.HeightMapResolution ||
                heightMap2D.format != GGeometry.HeightMapFormat)
            {
                CreateHeightMap2D();
            }
            return heightMap2D.GetPixelBilinear(pos.x, pos.y).r;
        }
