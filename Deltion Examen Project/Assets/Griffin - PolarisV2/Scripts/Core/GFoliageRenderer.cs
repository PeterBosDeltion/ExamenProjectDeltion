            ...
            for (i = 0; i < instanceCount; ++i)
            {
                ...
                if (sqrDistance < sqrBillboardStart)
                {
                    ...
                    for (d = 0; d < drawCallCount; ++d)
                    {
                        Graphics.DrawMesh(
                            p.SharedMesh,
                            Matrix4x4.TRS(worldPos + Vector3.up * p.PivotOffset, tree.Rotation, tree.Scale),
                            p.SharedMaterials[d],
                            p.Layer,
                            cam, //camera
                            d, //sub mesh index
        ...
        private static void RenderTreesInstanced_NonInstanced(GStylizedTerrain t, Camera cam, List<int> flags, List<Vector3> worldPositions)
        {
            ...
            for (int i = 0; i < instanceCount; ++i)
            {
                ...
                for (d = 0; d < drawCallCount; ++d)
                {
                    Graphics.DrawMesh(
                        proto.SharedMesh,
                        Matrix4x4.TRS(worldPositions[i] + Vector3.up * proto.PivotOffset, tree.Rotation, tree.Scale),
            ...
            for (int p = 0; p < prototypeCount; ++p)
            {
                for (int i = 0; i <= instanceCount; ++i)
                {
                    ...
                    if (tree.PrototypeIndex != p)
                        continue;
                    batchMatrices[batchInstanceCount] = Matrix4x4.TRS(worldPositions[i] + Vector3.up * prototypes[p].PivotOffset, tree.Rotation, tree.Scale);
